...
public class MonitorPanel extends JPanel {
     ...
     * 默认的Y轴线条数量
     */
    private static final int defaultYAxisLineCount = 10;

    // 动态Y轴，固定Y轴

    public static void main(String[] args) {
        long interval = 9440;
        double log = Math.log10(interval);// 计算位数，interval 的底数为 10 的对数。比如interval=100，则结果为2
        System.out.println(log);
        long _tmp = (long) Math.pow(10, log); // 10的log次方
        System.out.println(_tmp);

        long round = Math.round((double) interval / _tmp);
    ...
    private void calcYAxis() {
        ...
        if (interval == 0)
            interval = 1;
        long log = (long) Math.log10(interval);// 计算位数，interval 的底数为 10 的对数。比如interval=100，则结果为2

        long _tmp = (long) Math.pow(defaultYAxisLineCount, log); // 10的log次方
    ...
    public void drawImage() {
        ...
        if (tw != imgWidth || th != imgHeight) {
            ...
            imgWidth = tw;
            imgHeight = th;
            bimage = new BufferedImage(imgWidth, imgHeight, BufferedImage.TYPE_INT_RGB);
        ...
        big.drawLine(_xPoint, _yPoint, _xPoint + fm.stringWidth(valueOf), _yPoint);
    ...
    private void drawXAxis2() {
        ...
        if (clist.size() <= xLineCount) {
            for (int i = 0; i < clist.size() - 1; i++) {
                ...
                big.drawLine(_xPoint, titleHeight, _xPoint, titleHeight + graphHeight);
                big.setColor(fontColor);
                big.drawString(dateStr, _xPoint, titleHeight + graphHeight + ascent);

                String dateStr2 = dateF2.format(date);
                big.drawString(dateStr2, _xPoint, titleHeight + graphHeight + ascent + fHeight);
            }
        } else {
    ...
    private void drawYAxis() {
        ...
        for (int i = 0; i <= yAxisLineCount; i++) {
            ...
            // 数字
            big.setColor(fontColor);
            big.drawString(_yStr, YTextWidth - fm.stringWidth(_yStr) - 1, _yPosition + descent);

            // 画横线
            big.setColor(lineColor);
            big.drawLine(YTextWidth, _yPosition, YTextWidth + graphWidth, _yPosition);
        }
    }
